...
__global__ void sgemm_v4_nonvec(float * __restrict__ A,
...
template <const int BM, const int BN, const int BK,
            const int TM, const int TN>
__global__ void sgemm_v4_vec(float * __restrict__ A,
                        float * __restrict__ B,
                        float * __restrict__ C,
                        const int M,
                        const int K,
    ...
    auto load_ar = [&] (int k) {
        for (int i = 0; i < TM; i += 4) {
            FLOAT4(Ar[i]) = FLOAT4(AsT[k][i + thread_row]);
        }
    };
    auto load_br = [&] (int k) {
...
int main(int argc, char** argv) {
    ...
    for (int run = 0 ; run < nIter; run ++ ) {
        ...
        if (M % 4 == 0 && N % 4 == 0 && K % 4 == 0) {
            sgemm_v4_vec<BM, BN, BK, TM, TN><<<grid_size, block_size>>>(d_A, d_B, d_C, M, K, N);
        } else {
            sgemm_v4_nonvec<BM, BN, BK, TM, TN><<<grid_size, block_size>>>(d_A, d_B, d_C, M, K, N);
        }
    }
