...
__global__ void sgemm_v2(float * __restrict__ A,
    ...
    auto load_as = [&](int ph) {
        ...
        for (int i = tid; i < nelements; i += nthreads) {
            ...
            if (a_row < M && a_col < K) {
                As[shmem_row][shmem_col] = A[OFFSET(a_row, a_col, M, K)];
            } else {
                As[shmem_row][shmem_col] = 0.0f;
            }
        }
    };
...
int main(int argc, char** argv) {
    ...
    for (int run = 0 ; run < nIter; run ++ ) {
        ...
        sgemm_v2<BM, BN, BK, TM, TN><<<grid_size, block_size>>>(d_A, d_B, d_C, M, K, N);
    }
