    ...
    int M, int K, int N)
{
    ...
    float* AsT_ptr = smem;
    float* Bs_ptr  = AsT_ptr + 8 * 132;

    float (*AsT)[132] = (float (*)[132])AsT_ptr;
    float (*Bs)[128]  = (float (*)[128])Bs_ptr;
    float (*Cs)[16][36] = (float (*)[16][36])AsT_ptr;
...
int main(int argc, char** argv) {
    ...
    for (int run = 0 ; run < nIter; run ++ ) {
        ...
        if (M % 4 == 0 && N % 4 == 0 && K % 4 == 0) {
            sgemm_v7<<<grid_size, block_size, smem_size>>>(d_A, d_B, d_C, M, K, N);
        } else {
