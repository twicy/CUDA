...
__global__ void transpose_v4(float* A, float* B, int M, int N) {
    ...
    if (a_row < M && a_col < N) {
        As[thread_row][thread_col ^ thread_row] = A[OFFSET(a_row, a_col, M, N)];
    } else {
        As[thread_row][thread_col ^ thread_row] = 0.0f;
    }
    ...
    if (b_row < N && b_col < M) {
        B[OFFSET(b_row, b_col, N, M)] = As[thread_col][thread_row ^ thread_col];
    }
}
...
int main(int argc, char** argv) {
    ...
    for (int run = 0; run < nIter; run++) {
        transpose_v4<BLOCK_SIZE><<<grid_size, block_size>>>(d_B, d_A, M, N);
    }
