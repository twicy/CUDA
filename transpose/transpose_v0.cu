...
__global__ void transpose_v0(float* A, float* B, int M, int N) {
    int a_row = blockIdx.y * blockDim.y + threadIdx.y;
    int a_col = blockIdx.x * blockDim.x + threadIdx.x;
    if (a_row < M && a_col < N) {
        B[OFFSET(a_col, a_row, N, M)] = A[OFFSET(a_row, a_col, M, N)];
    }
}
...
int main(int argc, char** argv) {
    ...
    for (int run = 0; run < nIter; run++) {
        transpose_v0<<<grid_size, block_size>>>(d_B, d_A, M, N);
    }
